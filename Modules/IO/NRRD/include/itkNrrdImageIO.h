...
public:
  ...
  * ITK image can be an Image object that can store scalar, RGB, CovariantVector, etc. pixels in an arbitrarily high dimensional array.
  * If the number of components of the pixel has to be dynamic then the ITK image has to be a VectorImage object
  * that can store multi-component pixels in an arbitrarily high dimensional array.
  *
  * NRRD file can contain domain (space, time) axes and range (list, RGB, covariant-vector, etc.) axes in an arbitrary order.
  * Therefore, axes of the NRRD file may have to be reordered to be able to read the NRRD file into an ITK image.
  * In general, NRRD doman axes are mapped to ITK image axes then all range axes are used as additional image dimensions.
  * The only exception is that it makes sense to pick one a range axis and use that as pixel component.
  *
  * In early ITK versions, any kind of NRRD range axis was used as pixel component by default (this is the UseAnyRangeAxisAsPixel strategy).
  * However, this resulted in more complicated application code (developers need to instantiate an Image or VectorImage depending
  * on what kind of axes are in the NRRD file) and potential unnecessary reordering of axes (e.g., sequence of 3D images was always converted
  * into a single multi-component 3D image).
  *
  * Therefore additional strategies were introduced to allow more control over how NRRD axes are mapped to ITK image axes
  * and pixel component. UseNonListRangeAxisAsPixel to ensure that the image can be always read into an Image object.
  * UseScalarPixel to ensure that the image can be always read into an Image object with a scalar pixel type.
  *
  * \li UseAnyRangeAxisAsPixel Use the first a range axis (i.e., non-domain axis) of the NRRD file as pixel component.
  *     This is the default (for backward compatibility).
  *     If the NRRD file contains a non-list type (e.g., RGB, point, covariant-vector) range axis then the first one will be used as pixel component;
  *     the image will be read into an Image object.
  *     If the NRRD file only contains list-type range axis then it will be used as pixel component; the image will be read into a VectorImage.
  * \li UseNonListRangeAxisAsPixel Use the first non-list range axis (i.e., non-domain axis) of the NRRD file as pixel component.
  *     The image is always read into an Image object, with potentially a non-scalar pixel type.
  * \li UseScalarPixel Pixel type is scalar. All range (i.e., non-domain) axes of the NRRD file are added as additional domain axes.
  *     The image is always read into an Image object with a scalar pixel type.
  *
  * Examples:
  *
