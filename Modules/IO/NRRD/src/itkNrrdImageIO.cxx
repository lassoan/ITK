                           ...
                           itk::NrrdImageIOEnums::AxesReorder axesReorder)
{
  ...
  for (unsigned int rangeAxisIndex = 0; rangeAxisIndex < rangeAxisNum_nrrd; ++rangeAxisIndex)
  {
    ...
    if (rangeAxis_nrrd != pixelAxisIndex)
    {
...
NrrdImageIO::ReadImageInformation()
{
  ...
  try
  {
    ...
    if (nrrd->spaceDim > 0)
    {
      ...
      if (nrrd->spaceDim != numberOfDomainAxes)
      {
        itkExceptionMacro("ReadImageInformation: number of domain axes in the NRRD file ("
          << numberOfDomainAxes << ") doesn't match dimension of space in which orientation is defined ("
          << nrrd->spaceDim << "). This is not supported.");
      }
    }
    ...
    for (unsigned int domainAxis_itk = 0; domainAxis_itk < numberOfDomainAxes; ++domainAxis_itk)
    {
      ...
      switch (spacingStatus)
      {
        ...
        case nrrdSpacingStatusScalarWithSpace:
          // Both "spacings" and "space origin" fields are specified. This is an unusual combination, no need to support it.
          // Either use space fields ("space directions" and "space origin") or use older convention of just "spacings".
          itkExceptionMacro("ReadImageInformation: Error interpreting "
    ...
    if (pixelAxisIndex >= 0)
    {
      ...
      {
        std::string key =
          NRRD_KEY_PREFIX + airEnumStr(nrrdField, nrrdField_labels) + "[pixel]";
        EncapsulateMetaData<std::string>(thisDic, key, axisInfo->label);
      }
      if (airStrlen(axisInfo->units))
      {
...
NrrdImageIO::Write(const void * buffer)
{
  ...
  double        spaceDir[NRRD_DIM_MAX][NRRD_SPACE_DIM_MAX]{ AIR_NAN };
  ...
  for (unsigned int axi = 0; axi < this->GetNumberOfDimensions(); ++axi)
  {
    ...
    std::string  key =
      std::string(NRRD_KEY_PREFIX) + airEnumStr(nrrdField, nrrdField_kinds) + "[" + std::to_string(axi) + "]";
  ...
  if (nrrdWrap_nva(nrrd, const_cast<void *>(buffer), this->ITKToNrrdComponentType(m_ComponentType), nrrdDimensions, size) ||
      (spaceDim == 3
