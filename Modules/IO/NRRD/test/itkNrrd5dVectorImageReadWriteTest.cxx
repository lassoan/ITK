...
GenerateImageSequence(
  ...
  std::vector<typename ImageType::Pointer> & imageList)
{
  ...
  for (int timePoint = 0; timePoint < numberOfTimePoints; ++timePoint)
  {
    ...
    image->FillBuffer(ImageType::PixelType(timePoint * 2));
...
void
WriteImageSequenceFile(
  std::string filename,
  std::vector<typename itk::Image<typename PixelType, SpaceDimension>::Pointer> & imageList,
  bool listAxisFastest=false)
{
  ...
  typedef typename itk::JoinSeriesImageFilter<ImageType, ImageSequenceType> JoinImageFilterType;
  ...
  ImageSequenceType::Pointer joinedImage;
  ...
  {
    // axis order: 3 0 1 2
    //joinImageFilter->SetSpacing()

    joinImageFilter->Update();
    joinedImage = joinImageFilter->GetOutput();

    auto origin1 = joinedImage->GetOrigin();
    ...
    itk::FixedArray<unsigned int, ImageSequenceType::ImageDimension> order;
    order[0] = SpaceDimension;
    for (int i = 1; i < ImageSequenceType::ImageDimension; ++i)
    {
      order[i] = i - 1;
    }
    ...
    ImageSequenceType::PointType oldOrigin = joinedImage->GetOrigin();
    ImageSequenceType::PointType newOrigin;
    for (int i = 0; i < ImageSequenceType::PointType::Dimension; ++i)
    {
      newOrigin[i] = oldOrigin[order[i]];
    }
    joinedImage->SetOrigin(newOrigin);

    auto origin2 = joinedImage->GetOrigin();
  ...
  // Write to file
  typedef typename itk::ImageFileWriter<ImageSequenceType> ImageWriterType;
...
//----------------------------------------------------------------------------
void
CheckImageSequenceFileHeader(std::string filename, unsigned int SpaceDimension, unsigned int expectedDimensionInFile, std::string expectedPixelType, int expectedComponents, bool useNonListExtensionAsPixel=false)
{
  ...
  {
    throw itk::InvalidArgumentError("Unexpected pixel type in NRRD file: " +
                                    imageIO->GetPixelTypeAsString(imageIO->GetPixelType()) + " != " + expectedPixelType);
  }
  ...
  std::vector<typename itk::Image<typename PixelType, SpaceDimension>::Pointer> & imageList)
{
  ...
  reader->Update();
  ImageSequenceType::ConstPointer image = reader->GetOutput();
  ...
  ImageSequenceType::RegionType fullRegion = image->GetLargestPossibleRegion();
  ImageSequenceType::SizeType   extractionSize = fullRegion.GetSize();
  ...
  for (int timePoint = 0; timePoint < numberOfTimePoints; ++timePoint)
  {
    ImageSequenceType::RegionType extractionRegion;
    ImageSequenceType::IndexType  extractionIndex = extractionRegion.GetIndex();
...
template <class PixelType, int SpaceDimension>
void
ReadImageFile(std::string                                                                filename,
                      typename itk::VectorImage<typename PixelType, SpaceDimension>::Pointer & image)
{
...
itkNrrd5dVectorImageReadWriteTest(int argc, char * argv[])
{
  ...
  // We test if we can read and write an image sequence with the list axis as the slowest axis.
  // This is important because often the image sequence has to be accessed as individual 3D volumes.
  // In this case, we do not want to reorder the axes in the reader to make the time axis the fastest axis,
  // and then reorder it back to the original order when processing or writing the image.
  ...
  // 2D scalar image sequence, written and read with list as the slowest axis
  {
    ...
    std::vector<typename itk::Image<typename PixelType, SpaceDimension>::Pointer> imageListRead;
  ...
  // Read time points into vector pixel type
  ...
  // 3D displacement field sequence
  {
    ...
    GenerateImageSequence<ImageType>(20, imageListToWrite);
    WriteImageSequenceFile<PixelType, SpaceDimension>(filename, imageListToWrite);
    CheckImageSequenceFileHeader(filename, SpaceDimension, SpaceDimension + 1, expectedPixelType, SpaceDimension, true);
    std::vector<typename itk::Image<typename PixelType, SpaceDimension>::Pointer> imageListRead;
    ReadImageSequenceFile<PixelType, SpaceDimension>(filename, imageListRead);
  }
  ...
  // 2D displacement field sequence
  {
    ...
    GenerateImageSequence<ImageType>(20, imageListToWrite);
    WriteImageSequenceFile<PixelType, SpaceDimension>(filename, imageListToWrite);
    CheckImageSequenceFileHeader(filename, SpaceDimension, SpaceDimension + 1, expectedPixelType, SpaceDimension);
    std::vector<typename itk::Image<typename PixelType, SpaceDimension>::Pointer> imageListRead;
    ReadImageSequenceFile<PixelType, SpaceDimension>(filename, imageListRead);
  }
  ...
  // 2D color image (RGB) sequence
  {
    ...
    const int                                            expectedComponents = 3; // RGB has 3 components
    typedef itk::RGBPixel<double> PixelType;
    typedef itk::Image<PixelType, SpaceDimension>        ImageType;
    std::vector<ImageType::Pointer>                      imageListToWrite;
    GenerateImageSequence<ImageType>(20, imageListToWrite);
    WriteImageSequenceFile<PixelType, SpaceDimension>(filename, imageListToWrite);
    CheckImageSequenceFileHeader(filename, SpaceDimension, SpaceDimension + 1, expectedPixelType, expectedComponents);
    std::vector<typename itk::Image<typename PixelType, SpaceDimension>::Pointer> imageListRead;
    ReadImageSequenceFile<PixelType, SpaceDimension>(filename, imageListRead);
  }
  ...
  // 3D color image (RGBA) sequence
  {
    ...
    const int                                     expectedComponents = 4; // RGBA has 4 components
  ...
  // RGB color can be distinguished from displacement field
  {
    ...
    try
    {
      CheckImageSequenceFileHeader(filename, SpaceDimension, SpaceDimension + 1, expectedPixelType, 3);
    }
    catch (itk::InvalidArgumentError & e)
    {
      std::cout << "Caught expected error: " << e.what() << std::endl;
